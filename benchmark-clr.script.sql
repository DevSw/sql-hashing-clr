...
if OBJECT_ID('dbo.TestValuesSub8000') is not null
	drop table dbo.TestValuesSub8000;
create table dbo.TestValuesSub8000
(
...
/*	Populate the table with random NEWID() values.
	...
	each different lengths at multiples of 500. There are 15 multiples of 500 
	between 0 and 8000, so we end up with 15*6000=90000 values.
*/
declare @length int = 500;
while (@length < 8000)
BEGIN
	...
	BEGIN
		...
		insert into dbo.TestValuesSub8000 with (tablockx)
...
BEGIN
	...
	BEGIN
		...
		select 'SQL_' + @alg, cpu_time - @startCpu from sys.dm_exec_requests where session_id = @@SPID;
...
BEGIN
	...
	BEGIN
		...
		select dbo.GetHash(@alg,convert(varbinary(max),Value)) from dbo.TestValuesSub8000;
